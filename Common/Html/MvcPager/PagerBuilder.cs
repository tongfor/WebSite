...
namespace Common.Html
{
    internal class PagerBuilder
    {
        ...
        private string GenerateUrl(int pageIndex)
        {
            ...
            urlResult = !string.IsNullOrEmpty(_routeName) ? urlHelper.RouteUrl(_routeName, routeValues) : urlHelper.RouteUrl(routeValues);
            if (urlResult != null && (routeValues["action"].ToString().Equals("index", StringComparison.CurrentCultureIgnoreCase) && !urlResult.ToLower().Contains("/index")))
            {
                //如果action是Index，把省略的Index补上
                string[] urlArray = urlResult.Split('?');
                urlResult = string.Empty;
                if (urlArray.Length<=0 || urlArray.Length > 2)
                {
                    return urlResult;
                }
                for (int i=0;i<urlArray.Length;i++)
                {
                    urlResult += urlArray[i];
                    if (i == 0)
                    {
                        urlResult += "/Index?";
                    }
        ...
        private string GenerateAnchor(PagerItem item)
        {
            ...
            if (_pagerOptions.UseJqueryAjax)
            {
                ...
                if (!string.IsNullOrEmpty(_ajaxOptions.OnFailure) || !string.IsNullOrEmpty(_ajaxOptions.OnBegin) || (!string.IsNullOrEmpty(_ajaxOptions.OnComplete) && _ajaxOptions.HttpMethod.ToUpper() != "GET"))
                {
                    ...
                    scriptBuilder.Append("\',url:$(this).attr(\'href\'),IsSuccess:function(data,status,xhr){$(\'#");
